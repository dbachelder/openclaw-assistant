...
/**
 * Routes TTS requests to either local (device) or cloud (OpenAI, ElevenLabs, Fish Audio)
 * providers based on user settings.
 */
class TTSOrchestrator(
    ...
    /**
     * Speak the given text using the configured provider (single utterance).
    ...
    suspend fun speak(text: String): Boolean {
        ...
        return speakCloud(text, provider)
    }

    private suspend fun speakCloud(text: String, provider: String): Boolean = coroutineScope {
        val ttsProvider = createCloudProvider(provider)
        if (ttsProvider == null) {
            Log.w(TAG, "Cloud provider unavailable, falling back to local TTS")
            return@coroutineScope speakLocal(text)
        }

        val sentences = splitIntoSentences(text)
        if (sentences.isEmpty()) return@coroutineScope true
        ...
        try {
            for (sentence in sentences) {
                try {
                    ...
                    Log.w(TAG, "Cloud TTS synthesis failed for sentence, skipping", e)
                    allSucceeded = false
                }
            }
        } finally {
